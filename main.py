...
def main():
    ...
    group_option = st.sidebar.selectbox("Group by", options=["None", "GlobalId", "Name", "Type", "FileName"], key="group_option")
    ...
    for uploaded_file in uploaded_files:
        ...
        if st.sidebar.button('Export to Excel ' + uploaded_file.name):
            st.sidebar.markdown(get_excel_download_link(df, filename=f"{uploaded_file.name}.xlsx"), unsafe_allow_html=True)

        # Export to SQL Server button
        if st.sidebar.button('Export to SQL Server ' + uploaded_file.name) and server and username and password and database and table_name:
            connection_string = f'mssql+pyodbc://{username}:{password}@{server}/{database}?driver=ODBC+Driver+17+for+SQL+Server'
            engine = create_engine(connection_string)
            df.to_sql(table_name, engine, if_exists='append')  # append to existing table if it exists
            st.sidebar.write(f'Data from {uploaded_file.name} written to SQL Server')
